...
/* iSH-AOK: turned OFF by hand after ./configure, and it must stay off.
 * configure is talking about DARWIN's poll(2), which it is right about -- but
 * no OpenSSH source here ever reaches it. kernel/native_libc.h rewrites poll()
 * to AOK's own nlibc_poll, which speaks GUEST file descriptors; letting
 * openbsd-compat/bsd-poll.c define a replacement poll() instead just collides
 * with that rewrite (error: conflicting types for 'nlibc_poll').
 * bsd-poll.c still supplies ppoll(), which OpenSSH's client loop leans on
 * heavily and Darwin lacks, and that one is built on pselect() -- also routed
 * through native_libc.h, so it sees guest descriptors too.
 * Re-running ./configure regenerates this file and puts the #define back; the
 * build then fails on bsd-poll.c and this comment is where to look. */
/* #undef BROKEN_POLL */
...
/* AOK: this config.h was generated by running configure on DARWIN, and the
 * tree is also compiled for Linux. Everything guarded below is a BSD/Darwin
 * fact that is false on glibc:
 *
 *   strtonum, timingsafe_bcmp, memset_s   openbsd-compat has replacements, each
 ...
 *
 * Left true everywhere else, so the shipping build is unchanged. */
#if !defined(__linux__)
